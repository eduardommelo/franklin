...
class voicemute extends fk.Comando{

    constructor(franklin)
    {
        super(franklin, {
            nome_comando: 'voicemute',
            plugin_grupo: 'moderacao',
            alternativas: ['vm', 'vmute', 'mutarvoz'],
            label: ['<@mention/id>','[motivo]'],
            descricao_comando: 'Silenciar um membro do canal de voz.',
    ...
    async runComando(message, args)
    {
        ...
        {
            message.reply(`<:no:557599389292429315> **|** Você não pode se silenciar do canal de voz.`);
            message.channel.stopTyping(true)
            return;
        }
        if(!voiceMembro.voiceChannel)
        {
            message.reply(`<:no:557599389292429315> **|** Este membro não se encontra no canal de voz.`);
            message.channel.stopTyping(true)
            return;
        }
        //===========================
        ...
        */
        ...
        {
                voiceMembro.setMute(true, motivo).then( r =>{
                    const data = new Date();
                    message.reply(`<:yes:557599188419084289> **|**   \`[${data.getDate()}:${data.getMonth()+1}:${data.getFullYear()}]\`  Membro **${voiceMembro}** foi silenciado do canal de voz por: \`${motivo}\`.`);
                    message.channel.stopTyping(true);
                }).catch( err =>{
                    const data = new Date();
                    message.reply(`<:no:557599389292429315> **|** Não encontrei este membro para silenciar do canal.`);
                    message.channel.stopTyping(true)
                })
